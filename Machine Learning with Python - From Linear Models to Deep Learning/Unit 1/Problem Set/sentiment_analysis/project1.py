...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
#    VECTORIZED
    z = labels * (np.sum(theta * feature_matrix, axis = 1) + theta_0)
    return np.mean(np.where(z >= 1, 0, 1 - z))
    
#    NON-VECTORIZED
#    numRows = feature_matrix.shape[0]
#    hingeLossSum = 0
#    for i in range(numRows):
#        hingeLossSum += hinge_loss_single(feature_matrix[i, :], label[i], theta, theta_0)
#    hingeLossMean = hingeLossSum / numRows
#    return hingeLossSum
#pragma: coderesponse end
